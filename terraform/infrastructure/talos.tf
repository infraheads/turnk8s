...
data "talos_client_configuration" "cp_cc" {
  ...
  nodes                = [proxmox_vm_qemu.controlplane[var.cluster_name].default_ipv4_address]
  endpoints            = [proxmox_vm_qemu.controlplane[var.cluster_name].default_ipv4_address]
}

# Generates a machine configuration for the control plane (controlplane.yaml)
data "talos_machine_configuration" "cp_mc" {
  cluster_name       = data.talos_client_configuration.cp_cc.cluster_name
  machine_type       = "controlplane"
  cluster_endpoint   = "https://${proxmox_vm_qemu.controlplane[var.cluster_name].default_ipv4_address}:6443"
...
resource "talos_machine_configuration_apply" "cp_mca" {
  ...
  node                        = proxmox_vm_qemu.controlplane[var.cluster_name].default_ipv4_address
}
